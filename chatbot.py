...
system_message = SystemMessagePromptTemplate.from_template(
    "You are a helpful AI assistant. You give responses in only 100 words. You provide responses according to the context only."
)
...
with st.sidebar:
    ...
    if st.session_state["chat_history"]:
        for i, chat in enumerate(reversed(st.session_state["chat_history"])):
            st.markdown(f"**Q{len(st.session_state['chat_history']) - i}:** {chat['user'][:50]}...")
    else:
